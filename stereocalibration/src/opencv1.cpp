...
// or 1 for Bouguet stereo methods). Calibrate the cameras and display 
// the rectified results along with the computed disparity images.
//
static void
	  ...
	  for( int s = 1; s <= maxScale; s++ )
		{
		  ...
		  result = cv::findChessboardCorners( timg, cvSize(nx, ny),&temp[0], &count,CV_CALIB_CB_ADAPTIVE_THRESH |CV_CALIB_CB_NORMALIZE_IMAGE);
		  if( timg != img )
			cvReleaseImage( &timg );
